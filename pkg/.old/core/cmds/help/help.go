...
import (
	...
	"github.com/rwxrob/bonzai/pkg/core/pager"
...
func injectDynamic(x *bonzai.Cmd) {
	bonzai.FuncMap[`aka`] = func() string {
...
func printIfHave(thing, name, value any) {
	...
	mark.PrintEmph(to.String(value))
	fmt.Println()
}
...
// be in BonzaiMark markup (see [bonzai.Format]). Emphasis is omitted if the
// terminal is not interactive (see [mark.Emph]).
//
// ForTerminal will detect a system pager and use it if found.
func ForTerminal(x *bonzai.Cmd, section string) {
